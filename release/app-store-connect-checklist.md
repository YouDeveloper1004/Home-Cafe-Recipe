...
- App Store Connect App Privacy에는 이메일, 사용자 ID, 사용자 콘텐츠, 사진·영상, 진단 정보의 실제 처리와 목적을 현재 빌드 기준으로 정확히 선택한다.
